...
module.exports = function (session) {
  ...
  RedisStore.prototype.get = function (sid, fn) {
    ...
    store.client.get(psid, function (er, data) {
      ...
      console.log('session.redis.get', elapsed, 'ms');
      if (er) return fn(er);
      if (!data) return fn();
  ...
  RedisStore.prototype.set = function (sid, sess, fn) {
    ...
    if (store.disableTTL) {
      ...
      store.client.set(psid, jsess, function (er) {
        ...
        console.log('session.redis.set', elapsed, 'ms');
        if (er) return fn(er);
        debug('SET complete');
    ...
    store.client.setex(psid, ttl, jsess, function (er) {
      ...
      console.log('session.redis.setex', elapsed, 'ms');
      if (er) return fn(er);
  ...
  RedisStore.prototype.destroy = function (sid, fn) {
    ...
    this.client.del(sid, function(err, res) {
      ...
      console.log('session.redis.del', elapsed, 'ms');
      fn(err, res);
    });
  };
  ...
  RedisStore.prototype.touch = function (sid, sess, fn) {
    ...
    store.client.expire(psid, ttl, function (er) {
      ...
      console.log('session.redis.expire', elapsed, 'ms');
      if (er) return fn(er);
